...
class LightGCN(GeneralModel):
    ...
    def __init__(self, args, corpus):
        ...
        norm_adj_path = corpus.prefix + corpus.dataset + '_norm_adj.pkl'
        if os.path.exists(norm_adj_path):
            logging.info('Load norm_adj from {}'.format(norm_adj_path))
            self.norm_adj = pickle.load(open(norm_adj_path, 'rb'))
        else:
            self.norm_adj = self.build_adjmat(corpus.n_users, corpus.n_items, corpus.train_clicked_set)
            logging.info('Save norm_adj to {}'.format(norm_adj_path))
            pickle.dump(self.norm_adj, open(norm_adj_path, 'wb'))
        super().__init__(args, corpus)   # corpus is a BaseReader object !
